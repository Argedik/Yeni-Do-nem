...
/** Gündem metni yapıştırıldığı gibi gösterilir: numara eklenmez, satırlar korunur. */
function gundemMetniHTML(t) {
  const m = (t.gundemMetin ?? (t.gundem || []).join('\n')).trim();
  if (!m) return `<p class="ipucu" style="margin:0 0 14px">Henüz girilmedi.</p>`;
  return `<div style="margin:0 0 14px;line-height:1.7;white-space:pre-wrap">${kacik(m)}</div>`;
}
