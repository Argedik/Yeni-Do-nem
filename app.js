  ...
  return `<div class="kart">
    ...
    <div class="kart-ic">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:6px">
        <h4 style="font-size:13px;color:var(--soluk);text-transform:uppercase;letter-spacing:.5px;margin:0">Gündem maddeleri</h4>
        ${gundemDuzenlenen === t.id ? '' : `<button class="btn sm gri" data-act="gundem-duzenle" data-id="${t.id}">${(t.gundem || []).length ? 'Düzenle' : '+ Gündem yaz'}</button>`}
      </div>
      ${gundemDuzenlenen === t.id ? `
        <textarea id="gundem_metin_${t.id}" style="min-height:140px" placeholder="Gündem metnini yapıştır. Her satır bir madde; 📌 / 1. / - gibi işaretler temizlenir. İşaretsiz satırlar başlık olur.">${kacik(t.gundemMetin ?? (t.gundem || []).join('\n'))}</textarea>
...
function gundemYazdir(id) {
  ...
  yazdir(`<div class="p-ust">
    ...
    ${(() => { let n = 0; return gundemYapisi(t.gundemMetin ?? g.join('\n')).map(x => x.tip === 'baslik'
      ? `<h3 style="margin:14px 0 4px">${kacik(x.metin)}</h3>` : `<div style="margin:3px 0 3px 18px">${++n}. ${kacik(x.metin)}</div>`).join(''); })()}
...
function gundemListesiHTML(t) {
  const yapi = gundemYapisi(t.gundemMetin ?? (t.gundem || []).join('\n'));
  if (!yapi.length) return `<p class="ipucu" style="margin:0 0 14px">Henüz girilmedi.</p>`;
  let n = 0;
  return `<div style="margin:0 0 14px;line-height:1.7">${yapi.map(x => x.tip === 'baslik'
    ? `<div style="font-weight:700;margin-top:6px">${kacik(x.metin)}</div>`
    : `<div style="padding-left:14px">${++n}. ${kacik(x.metin)}</div>`).join('')}</div>`;
}
