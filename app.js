...
function varsayilanVeri() {
  return {
    ...
    isler: [
      { id: yeniId(), ad: 'Toplantı hatırlatma mesajını GM grubuna ilet', kategori: 'İletişim', tip: 'toplanti', ofset: 0, saat: '12:00', aciklama: 'Şablonlar sekmesindeki hazır metni kopyalayıp gruba at.', link: '' },
      { id: yeniId(), ad: 'Katılım mazeretlerini liste halinde ilet', kategori: 'İletişim', tip: 'toplanti', ofset: 0, saat: '17:00', aciklama: 'Toplantılar sekmesinde katılım işaretle → "Mazeret listesini kopyala".', link: '' },
      ...
      { id: yeniId(), ad: 'Görev değişikliği / yeni katılımda güncel listeyi düzenle', kategori: 'Liste', tip: 'surekli', aciklama: 'Genel merkez kaynaklı her değişiklikte listeyi ve Drive dosyasını güncelle.', link: '' },
    ],
    toplantilar: [],
    uyeler: [],
    linkler: [
    ...
    sablonlar: [
      {
        id: yeniId(), ad: 'GM grubu toplantı hatırlatması', metin:
          'Selamünaleyküm hanımlar 🌸\n\n{tarih} {gun} günü saat {saat}\'te {yer} adresinde birim toplantımız var.\n\nKatılamayacak olan hanımların bugün 17.00\'ye kadar bana mazeretlerini iletmesini rica ediyorum.\n\nHepimiz için hayırlı olsun 🤍'
      },
      {
        id: yeniId(), ad: 'Mazeret listesi başlığı', metin:
          '📌 {tarih} Toplantısı — Katılım Durumu\n\n{mazeretler}'
      },
      {
        id: yeniId(), ad: 'Tutanak üst bilgisi', metin:
          '{birim}\nTOPLANTI TUTANAĞI\n\nTarih: {tarih}\nSaat: {saat}\nYer: {yer}\nKatılım: {katilim}\n\nGündem:\n{gundem}\n\nKararlar:\n1. \n2. \n3. '
      },
    ],
    yapildi: {},   // "isId#YYYY-MM-DD" -> {t: zaman}
    surekli: {},   // isId -> son yapıldı ISO
    log: [],       // liste değişiklik günlüğü
  };
}
...
const gorunumler = {
  bugun: gorunumBugun, takvim: gorunumTakvim, rutin: gorunumRutin, toplanti: gorunumToplanti,
  liste: gorunumListe, sablon: gorunumSablon, drive: gorunumDrive, ayar: gorunumAyar,
};
...
function toplantiKarti(t) {
  const k = t.katilim || {};
  const aktifUye = state.uyeler.filter(u => u.durum !== 'ayrildi');
  const say = (v) => aktifUye.filter(u => k[u.id] === v).length;
  ...
  return `<div class="kart">
    ...
    <div class="kart-ic">
      ...
      <div class="rozet-sira" style="margin-bottom:12px">
        <span>✅ Katılan <b>${say('katildi')}</b></span><span>🟡 Mazeret <b>${say('mazeret')}</b></span>
        <span>❌ Katılmayan <b>${say('katilmadi')}</b></span><span>👥 Toplam <b>${aktifUye.length}</b></span>
      </div>
      ...
      <h4 style="font-size:13px;color:var(--soluk);text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">Katılım</h4>
      ${aktifUye.length ? aktifUye.map(u => `
        <div class="katilim-satir">
          <span class="ad">${kacik(u.ad)} ${u.gorev ? `<span class="gorev">· ${kacik(u.gorev)}</span>` : ''}</span>
          <div class="secenek">
            <button class="sec-katildi ${k[u.id] === 'katildi' ? 'aktif' : ''}" data-act="katilim" data-t="${t.id}" data-u="${u.id}" data-v="katildi">Katıldı</button>
            <button class="sec-mazeret ${k[u.id] === 'mazeret' ? 'aktif' : ''}" data-act="katilim" data-t="${t.id}" data-u="${u.id}" data-v="mazeret">Mazeret</button>
            <button class="sec-yok ${k[u.id] === 'katilmadi' ? 'aktif' : ''}" data-act="katilim" data-t="${t.id}" data-u="${u.id}" data-v="katilmadi">Gelmedi</button>
          </div>
          <input type="text" style="max-width:190px" placeholder="mazeret sebebi" value="${kacik((t.sebep || {})[u.id] || '')}"
            data-act-input="sebep" data-t="${t.id}" data-u="${u.id}">
        </div>`).join('')
      : `<p class="ipucu">Önce <b>Liste</b> sekmesinden üyeleri ekle, katılım burada işaretlenebilir.</p>`}
...
function toplantiFormuAc(id) {
  ...
  modalAc(id ? 'Toplantıyı düzenle' : 'Yeni toplantı', `
    ...
    <div class="form-satir"><div class="alan"><label>Tutanak dosya linki</label>
      <input type="url" id="t_tutanak" value="${kacik(t.tutanakLink || '')}" placeholder="https://docs.google.com/..."></div></div>
...
function toplantiKaydet(id) {
  ...
  else state.toplantilar.push({ id: yeniId(), katilim: {}, sebep: {}, ...kayit });
...
function donemKaydet() {
  ...
  for (let g = bas; gunFarki(g, bit) >= 0; g = gunEkle(g, 7 * aralik)) {
    ...
    state.toplantilar.push({ id: yeniId(), tarih: g, saat, yer, gundem: ['', '', ''], katilim: {}, sebep: {}, notlar: '', tutanakLink: '' });
    n++;
  }
...
function mazeretMetni(id) {
  ...
  const u = state.uyeler.filter(x => x.durum !== 'ayrildi');
  const grup = (v) => u.filter(x => k[x.id] === v);
  const yaz = (baslik, liste, sebepli) => liste.length
    ? `${baslik} (${liste.length}):\n` + liste.map((x, i) =>
      `${i + 1}. ${x.ad}${sebepli && s[x.id] ? ' — ' + s[x.id] : ''}`).join('\n') + '\n\n' : '';
  return `📌 ${kisaTarih(t.tarih)} Toplantısı — Katılım Durumu\n\n`
    + yaz('🟡 Mazeret bildiren', grup('mazeret'), true)
    + yaz('❌ Katılmayan', grup('katilmadi'), false)
    + yaz('✅ Katılan', grup('katildi'), false)
    + `👥 Toplam kayıtlı: ${u.length}`;
}
...
/* ================================================================
   GÖRÜNÜM: LİSTE (üyeler + değişiklik günlüğü)
   ================================================================ */
function gorunumListe() {
  const u = state.uyeler;
  const aktif = u.filter(x => x.durum !== 'ayrildi');
  return `
  <div class="kart">
    <div class="kart-bas"><h2>👥 Güncel liste <span class="sayi">${aktif.length} aktif</span></h2>
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        <button class="btn sm gri" data-act="liste-kopya">📋 Listeyi kopyala</button>
        <button class="btn sm" data-act="uye-duzenle" data-id="">+ Kişi ekle</button>
      </div></div>
    <div class="kart-ic sifir">
      <table><thead><tr><th>Ad Soyad</th><th>Görev</th><th>Telefon</th><th>Durum</th><th class="sag">İşlem</th></tr></thead>
      <tbody>${u.length ? u.map(x => `<tr>
        <td><b>${kacik(x.ad)}</b>${x.not ? `<div class="ipucu">${kacik(x.not)}</div>` : ''}</td>
        <td>${kacik(x.gorev || '—')}</td>
        <td>${x.tel ? `<a href="tel:${kacik(x.tel)}">${kacik(x.tel)}</a>` : '—'}</td>
        <td><span class="durum ${x.durum === 'ayrildi' ? 'ayrildi' : 'aktif'}">${x.durum === 'ayrildi' ? 'Ayrıldı' : 'Aktif'}</span></td>
        <td class="sag"><button class="btn sm gri" data-act="uye-duzenle" data-id="${x.id}">Düzenle</button></td></tr>`).join('')
      : `<tr><td colspan="5"><div class="bos">Henüz kimse eklenmedi. <b>+ Kişi ekle</b> ile başla.</div></td></tr>`}</tbody></table>
    </div></div>

  <div class="kart">
    <div class="kart-bas"><h2>🧾 Değişiklik günlüğü <span class="sayi">${state.log.length}</span></h2>
      <button class="btn sm gri" data-act="log-ekle">+ Not ekle</button></div>
    <div class="kart-ic sifir">
      ${state.log.length ? `<ul class="is-liste">${[...state.log].reverse().slice(0, 40).map(l => `<li class="is">
        <div class="is-govde"><div class="is-ad" style="font-weight:500">${kacik(l.metin)}</div>
        <div class="is-alt"><span class="etiket saat">${kisaTarih(l.tarih)}</span></div></div></li>`).join('')}</ul>`
      : `<div class="bos">Genel merkez görev değişikliği, yeni katılım vb. her düzenlemeyi buraya not et — yıl sonu raporunda işine yarar.</div>`}
    </div></div>`;
}

function uyeFormuAc(id) {
  const x = state.uyeler.find(u => u.id === id) || { ad: '', gorev: '', tel: '', durum: 'aktif', not: '' };
  modalAc(id ? 'Kişiyi düzenle' : 'Kişi ekle', `
    <div class="form-satir"><div class="alan"><label>Ad Soyad</label><input type="text" id="u_ad" value="${kacik(x.ad)}"></div></div>
    <div class="form-satir">
      <div class="alan"><label>Görev</label><input type="text" id="u_gorev" value="${kacik(x.gorev || '')}" placeholder="Örn: Birim Başkanı"></div>
      <div class="alan"><label>Telefon</label><input type="text" id="u_tel" value="${kacik(x.tel || '')}" placeholder="05.."></div>
    </div>
    <div class="form-satir">
      <div class="alan"><label>Durum</label><select id="u_durum">
        <option value="aktif" ${x.durum !== 'ayrildi' ? 'selected' : ''}>Aktif</option>
        <option value="ayrildi" ${x.durum === 'ayrildi' ? 'selected' : ''}>Ayrıldı</option></select></div>
      <div class="alan"><label>Not</label><input type="text" id="u_not" value="${kacik(x.not || '')}"></div>
    </div>`,
    `${id ? `<button class="btn tehlike" data-act="uye-sil" data-id="${id}">Sil</button>` : ''}
     <button class="btn gri" data-act="modal-kapat">Vazgeç</button>
     <button class="btn" data-act="uye-kaydet" data-id="${id || ''}">Kaydet</button>`);
}

function uyeKaydet(id) {
  const al = (x) => document.getElementById(x)?.value?.trim() ?? '';
  const ad = al('u_ad');
  if (!ad) { toast('⚠️ Ad boş olamaz'); return; }
  const kayit = { ad, gorev: al('u_gorev'), tel: al('u_tel'), durum: al('u_durum'), not: al('u_not') };
  const i = state.uyeler.findIndex(u => u.id === id);
  if (i >= 0) {
    const eski = state.uyeler[i];
    if (eski.gorev !== kayit.gorev) logEkle(`${ad}: görev "${eski.gorev || '—'}" → "${kayit.gorev || '—'}" olarak güncellendi.`);
    if (eski.durum !== kayit.durum) logEkle(`${ad}: durum "${kayit.durum === 'ayrildi' ? 'Ayrıldı' : 'Aktif'}" olarak güncellendi.`);
    state.uyeler[i] = { ...eski, ...kayit };
  } else {
    state.uyeler.push({ id: yeniId(), ...kayit });
    logEkle(`${ad} listeye eklendi${kayit.gorev ? ' (' + kayit.gorev + ')' : ''}.`);
  }
  kaydet(); modalKapat(); ciz(); toast('✔️ Kaydedildi');
}
function logEkle(metin) { state.log.push({ tarih: bugunISO(), metin }); }

function listeMetni() {
  const u = state.uyeler.filter(x => x.durum !== 'ayrildi');
  return `📋 ${state.ayar.birim} — Güncel Liste (${kisaTarih(bugunISO())})\n\n`
    + u.map((x, i) => `${i + 1}. ${x.ad}${x.gorev ? ' — ' + x.gorev : ''}${x.tel ? ' — ' + x.tel : ''}`).join('\n')
    + `\n\nToplam: ${u.length} kişi`;
}

/* ================================================================
   GÖRÜNÜM: ŞABLONLAR
   ================================================================ */
function gorunumSablon() {
  const bg = bugunISO();
  const y = state.toplantilar.filter(t => t.tarih >= bg).sort((a, b) => a.tarih.localeCompare(b.tarih))[0];
  return `
  <div class="serit bilgi">💡 <div>Metinlerdeki <b>{tarih} {saat} {yer} {gun} {birim} {mazeretler} {gundem} {katilim}</b> yerine sıradaki toplantının bilgileri otomatik yazılır. Kopyala → WhatsApp'a yapıştır.</div></div>
  ${y ? `<p class="ipucu" style="margin:-6px 0 14px">Kullanılan toplantı: <b>${uzunTarih(y.tarih)} · ${kacik(y.saat || state.ayar.toplantiSaat)}</b></p>`
      : `<div class="serit uyari">⚠️ <div>Yaklaşan toplantı yok; yer tutucular boş kalır.</div></div>`}
  <div class="kart"><div class="kart-bas"><h2>✉️ Hazır mesajlar <span class="sayi">${state.sablonlar.length}</span></h2>
    <button class="btn sm" data-act="sablon-duzenle" data-id="">+ Yeni şablon</button></div>
  <div class="kart-ic sifir">${state.sablonlar.map(s => `<div style="padding:14px 16px;border-bottom:1px solid var(--cizgi)">
    <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:8px">
      <b>${kacik(s.ad)}</b>
      <div style="display:flex;gap:6px">
        <button class="btn sm" data-act="sablon-kopya" data-id="${s.id}">📋 Kopyala</button>
        <button class="btn sm gri" data-act="sablon-duzenle" data-id="${s.id}">Düzenle</button>
      </div></div>
    <pre style="margin:0;white-space:pre-wrap;font-family:inherit;font-size:13.5px;color:var(--soluk);background:var(--zemin);padding:10px 12px;border-radius:9px">${kacik(sablonDoldur(s.metin))}</pre>
  </div>`).join('')}</div></div>`;
}

function sablonDoldur(metin) {
  const bg = bugunISO();
  const t = state.toplantilar.filter(x => x.tarih >= bg).sort((a, b) => a.tarih.localeCompare(b.tarih))[0];
  const u = state.uyeler.filter(x => x.durum !== 'ayrildi');
  const k = t?.katilim || {};
  const deger = {
    tarih: t ? kisaTarih(t.tarih) : '…',
    gun: t ? GUN_ADLARI[tarihNesnesi(t.tarih).getDay()] : '…',
    saat: t ? (t.saat || state.ayar.toplantiSaat) : state.ayar.toplantiSaat,
    yer: (t?.yer || state.ayar.toplantiYer) || '…',
    birim: state.ayar.birim,
    gundem: t ? ((t.gundem || []).filter(Boolean).map((g, i) => `${i + 1}. ${g}`).join('\n') || '…') : '…',
    katilim: t ? `${u.filter(x => k[x.id] === 'katildi').length}/${u.length}` : '…',
    mazeretler: t ? mazeretMetni(t.id).split('\n').slice(2).join('\n').trim() : '…',
  };
  return metin.replace(/\{(\w+)\}/g, (m, a) => deger[a] ?? m);
}

function sablonFormuAc(id) {
  const s = state.sablonlar.find(x => x.id === id) || { ad: '', metin: '' };
  modalAc(id ? 'Şablonu düzenle' : 'Yeni şablon', `
    <div class="form-satir"><div class="alan"><label>Şablon adı</label><input type="text" id="s_ad" value="${kacik(s.ad)}"></div></div>
    <div class="form-satir"><div class="alan"><label>Metin</label>
      <textarea id="s_metin" style="min-height:180px">${kacik(s.metin)}</textarea>
      <span class="ipucu">Yer tutucular: {tarih} {gun} {saat} {yer} {birim} {gundem} {katilim} {mazeretler}</span></div></div>`,
    `${id ? `<button class="btn tehlike" data-act="sablon-sil" data-id="${id}">Sil</button>` : ''}
     <button class="btn gri" data-act="modal-kapat">Vazgeç</button>
     <button class="btn" data-act="sablon-kaydet" data-id="${id || ''}">Kaydet</button>`);
}
  ...
  <div class="kart"><div class="kart-bas"><h2>📊 Durum</h2></div>
    <div class="kart-ic">
      <div class="rozet-sira">
        ...
        <span>Kişi <b>${state.uyeler.length}</b></span>
        <span>Tamamlanan iş <b>${Object.keys(state.yapildi).length}</b></span>
        <span>Günlük kaydı <b>${state.log.length}</b></span>
      </div>
    </div></div>`;
}
...
// Katılım sebebi yazımı
document.body.addEventListener('input', e => {
  const s = e.target.closest('[data-act-input="sebep"]'); if (!s) return;
  const t = state.toplantilar.find(x => x.id === s.dataset.t); if (!t) return;
  t.sebep = t.sebep || {}; t.sebep[s.dataset.u] = e.target.value; kaydet();
});
...
document.body.addEventListener('click', e => {
  ...
  switch (act) {
    ...
    case 'katilim': {
      const t = state.toplantilar.find(x => x.id === b.dataset.t); if (!t) return;
      t.katilim = t.katilim || {};
      t.katilim[b.dataset.u] = t.katilim[b.dataset.u] === b.dataset.v ? '' : b.dataset.v;
      kaydet(); ciz(); break;
    }
    case 'mazeret-kopya': kopyala(mazeretMetni(id)); break;
    case 'gundem-yazdir': gundemYazdir(id); break;

    case 'uye-duzenle': uyeFormuAc(id); break;
    case 'uye-kaydet': uyeKaydet(id); break;
    case 'uye-sil': {
      const u = state.uyeler.find(x => x.id === id);
      if (u && confirm(`${u.ad} listeden tamamen silinsin mi? (Geçmiş toplantı kayıtlarından da düşer — "Ayrıldı" işaretlemek daha güvenli.)`)) {
        state.uyeler = state.uyeler.filter(x => x.id !== id);
        logEkle(`${u.ad} listeden silindi.`); kaydet(); modalKapat(); ciz(); toast('🗑️ Silindi');
      } break;
    }
    case 'liste-kopya': kopyala(listeMetni()); break;
    case 'log-ekle': {
      const m = prompt('Değişiklik notu (örn: "Ayşe Y. birim başkan yardımcısı olarak atandı"):');
      if (m && m.trim()) { logEkle(m.trim()); kaydet(); ciz(); toast('✔️ Not eklendi'); } break;
    }

    case 'sablon-duzenle': sablonFormuAc(id); break;
    case 'sablon-kaydet': {
      const ad = document.getElementById('s_ad').value.trim(), metin = document.getElementById('s_metin').value;
      if (!ad) { toast('⚠️ Ad boş olamaz'); return; }
      const i = state.sablonlar.findIndex(x => x.id === id);
      if (i >= 0) state.sablonlar[i] = { ...state.sablonlar[i], ad, metin };
      else state.sablonlar.push({ id: yeniId(), ad, metin });
      kaydet(); modalKapat(); ciz(); toast('✔️ Kaydedildi'); break;
    }
    case 'sablon-kopya': {
      const s = state.sablonlar.find(x => x.id === id); if (s) kopyala(sablonDoldur(s.metin)); break;
    }
    case 'sablon-sil':
      if (confirm('Şablon silinsin mi?')) {
        state.sablonlar = state.sablonlar.filter(x => x.id !== id);
        kaydet(); modalKapat(); ciz();
      } break;
